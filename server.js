...
const server = http.createServer(async (req, res) => {
    ...
    function streamWithFFmpeg(playlistUrl) {
        let hb = null;
        ...
        const headers = [
            ...
            `Referer: ${playlistUrl}`,
            `Origin: ${new URL(playlistUrl).origin}`
        ].join('\r\n');
        const args = [
        ...
        abortCurrentStream = () => { try { child.kill('SIGTERM'); } catch {}; if (hb) { try { clearInterval(hb); } catch {} } try { res.end(); } catch {} };
        res.on('close', () => { abortCurrentStream(); });
        ...
        child.on('error', async () => {
            ...
            if (!headerSent && !res.headersSent) {
                await streamM3u8AsTs(playlistUrl);
                return;
            }
            try { res.end(); } catch {}
        });
    ...
    const preferFFmpeg = (reqUrl.searchParams.get('ff') === '1' || reqUrl.searchParams.get('transmux') === '1' || (process.env.FORCE_FFMPEG === '1'));
    if ((forceFormat === 'ts' || (!forceFormat && reqIsTs)) && (targetUrl.includes('.m3u8'))) {
        if (preferFFmpeg) { streamWithFFmpeg(targetUrl); return; }
